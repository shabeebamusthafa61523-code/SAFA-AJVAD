...
export default function PageFour({ onRSVPClick }) {
  ...
  return (
    <section className="scroll-section w-full z-20">
      <div className="w-full max-w-md mx-auto flex flex-col items-center justify-center min-h-screen text-center px-4 space-y-4 py-16">
        ...
        <motion.div initial="hidden" whileInView="visible" variants={slideUp} className="w-full px-6 py-8 rounded-[2rem] glass-card flex flex-col items-center shadow-md">
          ...
          <span className="block font-serif text-gray-900 text-3xl font-bold mb-2 tracking-wide drop-shadow-sm">Thavakkal Convention Centre</span>
          <span className="block font-sans text-gray-600 text-xs tracking-wider uppercase font-bold mb-5">Malappuram</span>

          <div className="w-full h-52 rounded-2xl overflow-hidden mb-5 shadow-sm border border-white/60">
            <iframe 
        ...
        {/* Actions Card */}
        <motion.div initial="hidden" whileInView="visible" variants={slideUp} className="w-full px-6 py-8 rounded-[2rem] glass-card flex flex-col items-center space-y-4 shadow-md">
          <button onClick={onRSVPClick} className="btn-primary w-full py-4 text-base">
            <CheckCircle size={20} /> Confirm Presence (RSVP)
          </button>
          
          <a href={googleCalendarUrl} target="_blank" rel="noopener noreferrer" className="btn-secondary w-full py-4 text-base mt-0">
        ...
        <motion.div initial="hidden" whileInView="visible" variants={slideUp} className="w-full px-6 py-8 rounded-[2rem] glass-card flex flex-col items-center shadow-md">
          <p className="font-sans text-[11px] tracking-[0.2em] font-bold text-[#A86D33] uppercase mb-2">Sharing the Happiness</p>
          <p className="font-sans text-sm text-gray-700 leading-relaxed font-bold tracking-wide">
